...
- **Upstream**: `majd/ipatool@v2.3.0` (tag, commit `19ffd1b`)
- **Applied patch**: PR #493 commit `a98f833` by @b-nnett (trivial conflict in `appstore_bag_test.go` resolved)
- **Fork tags**: see `git tag --list 'v2.3.1-fix-auth.*'` for available versions

> **Why v2.3.0 not main HEAD?** Main HEAD (`dcddce4`) switched from `github.com/99designs/keyring` to `github.com/byteness/keyring`, which is a breaking change for consumers that depend on `99designs/keyring` (e.g., `ipa-manager`). Forking from v2.3.0 preserves the `99designs/keyring` dependency.
